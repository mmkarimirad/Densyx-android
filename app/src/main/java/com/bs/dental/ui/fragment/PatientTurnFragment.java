...
import com.bs.dental.ui.adapter.PatientTurnAdapter;
...
/**
 * Created by mmkr on 23-Feb-17.
 * This filter is for nopCommerce version 3.7 and earlier
 */

public class PatientTurnFragment extends BaseFragment {

    private static final String CATEGORY_NAME = "categoryName";
    private static final String CATEGORY_ID = "categoryId";
    @InjectView(R.id.list_product) RecyclerView listProduct;
    ...
    protected PatientTurnAdapter patientAdapter;
    ...
    public static PatientTurnFragment newInstance(String categoryName, int categoryId) {
        PatientTurnFragment fragment = new PatientTurnFragment();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        if (rootView == null) {
            rootView = inflater.inflate(R.layout.fragment_patient_custom_turn, container, false);
            patientList = null;
        } else {
    ...
    public void callWebService() {
        if (patientAdapter != null) {
            patientAdapter.resetList();
        }
        // TODO: 5/17/2019 --- mmkr : remove comment after turn web service correction
        /*RetroClientTurn.getApi().getProductTurn(preferenceService.GetPreferenceValue(PreferenceService.COOKIE_TURN))
                .enqueue(new CustomCB<ProductTurnObject>(rootViewRelativeLayout));*/
        RetroClientTurn.getApi().getPatientTurns(preferenceService.GetPreferenceValue(PreferenceService.COOKIE_TURN))
                .enqueue(new CustomCB<PatientTurnObject>(rootViewRelativeLayout));
    }
    ...
    public void onEvent(PatientTurnObject response) {
        if (response != null /*&& response.getStatusCode() == 200*/) {

            List<PatientTurn> responsePatients = response.getData();
            if (responsePatients != null && responsePatients.size() > 0) {
                if (patientAdapter == null ) {

                    listProduct.setLayoutManager(layoutManager);
                    patientAdapter = new PatientTurnAdapter(getContext(), responsePatients, listProduct, 1);
                    listProduct.setAdapter(patientAdapter);
                    ...
                    if (resetList) {
                        ...
                        int start = patientAdapter.getItemCount();
                        int size = responsePatients.size();
                        patientAdapter.notifyItemRangeChanged(start, size);
                    }
                }
            } else if (responsePatients != null && responsePatients.size() == 0 /*&& patientAdapter != null && patientAdapter.getItemCount() > 0*/) {
                //patientAdapter.setLoaded();
                //patientAdapter.hideLoader();
                showToast(getString(R.string.not_item_to_process));
            }

        } else if (patientAdapter != null && patientAdapter.getItemCount() > 0) {
            patientAdapter.setLoaded();
            patientAdapter.hideLoader();
        }
    }
